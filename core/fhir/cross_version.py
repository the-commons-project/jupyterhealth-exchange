...
class _Engine:
    ...
    def _run_rule(self, rule, svars, tvars, depth):
        ...
        if element is None:
            ...
            json_key = _choice_key(element, source.get("type"))
        ...
        for binding in bindings:
            ...
            for dependent in rule.get("dependent", []):
                self._run_dependent(dependent, local_s, local_t, depth)
            for child in rule.get("rule", []):
    ...
    def _eval_condition(self, condition, svars, binding):
        # Supports ``<path> = 'literal'`` / ``<path> != 'literal'`` / ``<path> = <var>``.
        # Anything richer (FHIRPath ``.all()``, ``as`` casts) is unsupported -> the rule is
        # skipped (a documented lossy edge).
...
def _choice_key(element, type_hint):
    """Flatten a choice element to its FHIR-JSON key: ``effective`` + ``dateTime`` ->
    ``effectiveDateTime``. Non-choice elements (no type hint) are returned unchanged."""
    if element and type_hint:
        return element + type_hint[:1].upper() + type_hint[1:]
    return element
