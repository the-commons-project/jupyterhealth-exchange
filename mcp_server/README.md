...
- **Upstream failures are 5xx, not 401.** If JHE cannot be reached or misanswers the userinfo/introspection call, the server returns `503` (transient, with `Retry-After`) or `500` (misconfiguration) with no `WWW-Authenticate` header, so clients do not discard a valid token. `401` is reserved for tokens JHE actually rejects.
