...
- **Follows:** RFC 0002 §5's resolution; provenance per RFC 0001
...
has never been operable in any deployment (RFC 0002 §5), so there is nothing
to preserve.
...
| Domain restriction | `auth.sso.valid_domains` seeded but **never read** | enforced at first-time signup (not per-login; revoke = deactivate the user) |
...
`auth.default_orgs`), `identifier` = SAML NameID/uid. No asserted email →
signup fails closed ("Sign Up Closed", never the type-any-email form).
`SOCIALACCOUNT_EMAIL_VERIFICATION = "none"` (the IdP asserts the email);
`SOCIALACCOUNT_LOGIN_ON_GET = True` (one-click from our own button).

**Existing accounts:** `SOCIALACCOUNT_EMAIL_AUTHENTICATION` (+`_AUTO_CONNECT`)
is on so a practitioner with an existing password account logs straight in
via SAML and gets linked — without it, allauth's enumeration-safe default
dead-ends every existing same-email user on a fake "verification e-mail
sent" page. It only activates for IdPs whose SocialApp declares
`"verified_email": true` (§3), keeping IdP email trust an explicit per-IdP
decision. (Also fixed en route: `ACCOUNT_USER_MODEL_USERNAME_FIELD = None`,
required for JHE's username-less user model on allauth's social save path.)
...
1. Django admin → Social applications → add: provider `saml`, a name,
   ...
   mapping, and optional SP cert/key go in the settings JSON
   ([allauth SAML docs](https://docs.allauth.org/en/latest/socialaccount/providers/saml.html)).
   Add `"verified_email": true` if the IdP's email assertions are trusted —
   required for existing password accounts to log in via SAML (§2).
...
- `SOCIALACCOUNT_LOGIN_ON_GET = True` allows GET-initiated login —
  acceptable for a redirect flow from our own login page.
- Email authentication makes the per-IdP `"verified_email"` declaration
  security-relevant: only set it for IdPs that verify mailbox ownership.
...
## 6. Provenance (per RFC 0001's process)
