# CapabilityStatement source of truth (fhir_config vs. code) + AI-work provenance

- **RFC:** [jupyterhealth-exchange#704](https://github.com/jupyterhealth/jupyterhealth-exchange/issues/704) — discussion and acceptance live on the issue, via the `.github/ISSUE_TEMPLATE/02_rfc.yml` flow (the one used for #334 → #391): a maintainer signs off in the issue and opens/merges the implementation. This file carries no status of its own.
- **Companion PR:** #676 (CapabilityStatement + smart-configuration) · **Origin issue:** #615
- **Purpose:** design record + provenance manifest for #676. RFCs here are keyed by their GitHub issue number, not an ordinal.
