...
class FHIRBundlePagination(pagination.BasePagination):
    """
    ...
    def paginate_raw_sql(self, sql, params, request, count_sql=None):
        ...
        count_query_with_alias = f"SELECT COUNT(*) AS count_col FROM ({sql}) AS subquery" if not count_sql else f"{count_sql} AS count_col"
        ...
        results = [
            {columns[i]: getattr(row, columns[i]) for i in range(len(columns))}
