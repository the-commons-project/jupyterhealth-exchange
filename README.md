...
> **Quick local Setup:** For local development, Skip the steps 8–12 — as the `seed_db` command will creates the OAuth2 application. Also, Pre‑generated values of  `OIDC_RSA_PRIVATE_KEY`, `PATIENT_AUTHORIZATION_CODE_CHALLENGE`, and `PATIENT_AUTHORIZATION_CODE_VERIFIER` are provided in `env_example.txt` for dev/demo use only.
...
> **Production warning:** The `OIDC_RSA_PRIVATE_KEY`, `PATIENT_AUTHORIZATION_CODE_CHALLENGE`, and `PATIENT_AUTHORIZATION_CODE_VERIFIER` provided in `env_example.txt` are static demo keys for local development only and must **not** be used in production.
